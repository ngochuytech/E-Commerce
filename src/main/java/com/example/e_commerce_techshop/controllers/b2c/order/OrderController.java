...
public class OrderController {
    ...
    public ResponseEntity<?> getOrderById(@PathVariable String orderId) {
        try {
            ...
            return ResponseEntity.ok(ApiResponse.ok("Lấy thông tin đơn hàng thành công!", order));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(ApiResponse.error(e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> getOrdersByStoreAndStatus(@PathVariable String storeId, @PathVariable String status) {
        try {
            ...
            return ResponseEntity.ok(ApiResponse.ok("Lấy danh sách đơn hàng theo trạng thái thành công!", orders));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(ApiResponse.error(e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> getRecentOrders(@PathVariable String storeId, @RequestParam(defaultValue = "10") int limit) {
        try {
            ...
            return ResponseEntity.ok(ApiResponse.ok("Lấy danh sách đơn hàng gần đây thành công!", orders));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(ApiResponse.error(e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> getOrderStatistics(@PathVariable String storeId) {
        try {
            ...
            return ResponseEntity.ok(ApiResponse.ok("Lấy thống kê đơn hàng thành công!", 
    ...
    public ResponseEntity<?> getOrdersByDateRange(@PathVariable String storeId, 
        ...
        try {
            ...
            return ResponseEntity.ok(ApiResponse.ok("Lấy đơn hàng theo khoảng thời gian thành công!", orders));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(ApiResponse.error(e.getMessage()));
        }
    }
