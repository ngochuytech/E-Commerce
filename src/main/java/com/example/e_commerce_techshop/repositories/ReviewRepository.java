...
public interface ReviewRepository extends JpaRepository<Review, String> {
    ...
    @Query("SELECT r FROM Review r JOIN r.productVariant pv JOIN pv.product p WHERE p.storeId = :storeId")
    List<Review> findByStoreId(@Param("storeId") String storeId);
    
    // Find reviews by product
    @Query("SELECT r FROM Review r WHERE r.productVariantId IN (SELECT pv.id FROM ProductVariant pv WHERE pv.productId = :productId)")
    ...
    @Query("SELECT COUNT(r) FROM Review r JOIN r.productVariant pv JOIN pv.product p WHERE p.storeId = :storeId")
    Long countByStoreId(@Param("storeId") String storeId);
    
    // Get average rating by store
    @Query("SELECT AVG(r.rating) FROM Review r JOIN r.productVariant pv JOIN pv.product p WHERE p.storeId = :storeId")
    Double getAverageRatingByStoreId(@Param("storeId") String storeId);
    
    // Get reviews without seller response
    @Query("SELECT r FROM Review r JOIN r.productVariant pv JOIN pv.product p WHERE p.storeId = :storeId AND r.sellerResponse IS NULL")
