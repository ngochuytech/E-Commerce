...
public class StoreOrderService implements IStoreOrderService {
    ...
    private OrderDTO.OrderItem convertToOrderItemDTO(OrderItem orderItem) {
        ProductVariant productVariant = productVariantRepository.findById(orderItem.getProductVariant().getId()).orElse(null);
        ...
        return OrderDTO.OrderItem.builder()
                ...
                .price(BigDecimal.valueOf(orderItem.getPrice().longValue()))
                .subtotal(BigDecimal.valueOf(orderItem.getPrice()).multiply(BigDecimal.valueOf(orderItem.getQuantity())))
                .productId(productId)
                .category(category)
                .brand(brand)
                .stock(stock)
