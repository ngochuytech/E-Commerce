...
public class RegionalShippingService {
    ...
    /**
     * Tính số ngày giao hàng dự kiến dựa trên khoảng cách vùng (Shipment.Address)
     * 
     * @param fromAddress Địa chỉ gửi hàng (Shipment.Address)
     * @param toAddress   Địa chỉ nhận hàng (Shipment.Address)
     * @return Số ngày giao hàng dự kiến (1-5 ngày)
     */
    public int calculateEstimatedDeliveryDays(Shipment.Address fromAddress, Shipment.Address toAddress) {
        if (fromAddress == null || toAddress == null) {
            log.warn("Address is null, using default delivery time: 3 days");
            return 3;
        }
        ...
        // 1. Cùng tỉnh/thành - 1 ngày
        if (fromProvince.equalsIgnoreCase(toProvince)) {
            log.info("Estimated delivery (same city): 1 day");
            return 1;
        }
        ...
        if (fromRegion == null || toRegion == null) {
            log.warn("Cannot determine region for: {} -> {}, using default: 3 days", fromProvince, toProvince);
            return 3;
        }

        // 3. Tính số ngày theo khoảng cách vùng
        int estimatedDays = calculateDaysByRegion(fromRegion, toRegion);
        log.info("Estimated delivery: {} ({}) -> {} ({}): {} days",
                fromProvince, fromRegion, toProvince, toRegion, estimatedDays);

        return estimatedDays;
    }
    
    /**
     * Tính số ngày giao hàng dự kiến dựa trên khoảng cách vùng (Address)
     * 
     * @param fromAddress Địa chỉ gửi hàng (Address)
     * @param toAddress   Địa chỉ nhận hàng (Address)
     * @return Số ngày giao hàng dự kiến (1-5 ngày)
     */
    public int calculateEstimatedDeliveryDays(Address fromAddress, Address toAddress) {
        if (fromAddress == null || toAddress == null) {
            log.warn("Address is null, using default delivery time: 3 days");
            return 3;
        }
    ...
    /**
     * Tính số ngày giao hàng theo khoảng cách vùng
     */
    private int calculateDaysByRegion(String fromRegion, String toRegion) {
        // Cùng vùng - 2 ngày
        if (fromRegion.equals(toRegion)) {
            return 2;
        }

        // Bắc <-> Trung hoặc Trung <-> Nam (vùng lân cận) - 3 ngày
        if ((fromRegion.equals("NORTH") && toRegion.equals("CENTRAL")) ||
                (fromRegion.equals("CENTRAL") && toRegion.equals("NORTH")) ||
                (fromRegion.equals("CENTRAL") && toRegion.equals("SOUTH")) ||
                (fromRegion.equals("SOUTH") && toRegion.equals("CENTRAL"))) {
